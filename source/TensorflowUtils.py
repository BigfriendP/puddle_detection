...
def weight_variable(shape, stddev=0.02, name=None):
    # print(shape)
    initial = tensorflow.random.truncated_normal(shape, stddev=stddev)
    if name is None:
...
def RA_unit(x, h, w, n):
    ...
    x_t_small = tf.zeros([1, x_1.shape[1], int(w/2), 0], tf.float32)
    for k in range(n):
        x_t_1 = tf.slice(x_1, [0,k,0,0], [1,1,int(w/2),x.shape[3]])
